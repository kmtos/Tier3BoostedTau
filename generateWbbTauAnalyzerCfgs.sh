...
#input file prefix
inputFilePrefix="file:/mnt/hadoop/store/user/kmtos/NMSSM_Files/WbbJetsToLNu_Massive_TuneZ2star_8TeV-madgraph-pythia6_tauola-Summer12_DR53X-PU_S10_START53_V7A-v1-AODSIM_skim_v3/"
...
tauAnalyzerOutputFilePrefix="file:/home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/muHad_files/"

#EDM output file prefix
EDMOutputFilePrefix="file:/mnt/hadoop/store/user/kmtos/data1/`whoami`/Wbb/EDM_files/"
...
if [ $reweightOnly -eq 0 ]
    ...
    echo "cmsRun fileName_nonIso_cfg.py DONE"
    echo "List of files to be copied in nonIsoTauAnalyzerOutputFiles in if statement: "
    ls -1 ${nonIsoTauAnalyzerOutputFiles[${i}]}
    cp -f ${nonIsoTauAnalyzerOutputFiles[${i}]} /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/
...
echo "cmsRun fileName_nonIsoReweight_cfg.py DONE"
echo "List of files to be copied in isoTauAnalyzerOutputFiles: "
ls -1 ${isoTauAnalyzerOutputFiles[${i}]}
cp -f ${isoTauAnalyzerOutputFiles[${i}]} /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/
echo 'Copied {isoTauAnalyzerOutputFiles[${i}]}'
echo "List of files to be copied in nonIsoReweightTauAnalyzerOutputFiles: "
ls -1 ${nonIsoReweightTauAnalyzerOutputFiles[${i}]}
cp -f ${nonIsoReweightTauAnalyzerOutputFiles[${i}]} /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/
echo 'Copied {nonIsoReweightTauAnalyzerOutputFiles[${i}]}'
rm ${isoTauAnalyzerOutputFiles[${i}]} ${nonIsoReweightTauAnalyzerOutputFiles[${i}]} 
echo "Removed both of the copied sets of files"
...
cp -f ${allTauAnalyzerOutputFiles[${i}]} /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/
echo "Copying allTauAnalyzerOutputFiles[${i}]} DONE"
...
echo \"generateWbbTauAnalyzerCfgs.sh\"
condor_submit  Csubmit_\$jobName
done
exit 0" >> submitWbbTauAnalyzerJobs.sh
...
for sample in \`seq `expr $iBeg + 1` `expr $iEnd + 1`\`
  ...
  for cut in Iso NonIso NonIsoReweight
    ...
    if [ "\$cut" != "NonIso" ] || [ $reweightOnly -eq 0 ]
        then
        cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/muHad\${cut}Analysis_Wbb_${version}.root /mnt/hadoop/store/user/kmtos/data1/`whoami`/Wbb/analysis/
        rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/muHad\${cut}Analysis_Wbb_${version}.root
    fi
  done
done
...
for sample in \`seq `expr $iBeg + 1` `expr $iEnd + 1`\`
  do
  cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/muHadAnalysis_Wbb_${version}.root /mnt/hadoop/store/user/kmtos/data1/`whoami`/Wbb/analysis/
  rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/boosted-tau-analysis/BoostedTauAnalysis/TauAnalyzer/test/muHadAnalysis_Wbb_${version}.root
done
