...
#input file prefix
inputFilePrefix="file:/mnt/hadoop/store/user/kmtos/NMSSM_Files/WW_TuneZ2star_8TeV_pythia6_tauola-Summer12_DR53X-PU_S10_START53_V7A-v1-AODSIM_skim_v3/data_no_selection_"
...
EDMOutputFilePrefix="file:/mnt/hadoop/store/user/kmtos/data1/`whoami`/WW/EDM_files/"
...
for i in `seq $iBeg $iEnd`
  ...
  sed -e "s%FILES%${inputFileBlocks[${i}]}%" -e "s%CLEANJETSOUTFILE%${cleanJetsOutFiles[${i}]}%" -e "s%HIGHMTNONISOTAUANALYZEROUTFILE%${highMTNonIsoTauAnalyzerOutputFiles[${i}]}%" -e "s%HIGHMTALLTAUANALYZEROUTFILE%${highMTAllTauAnalyzerOutputFiles[${i}]}%" -e "s%HIGHMTISOTAUANALYZEROUTFILE%${highMTIsoTauAnalyzerOutputFiles[${i}]}%" -e "s%LOWMTNONISOTAUANALYZEROUTFILE%${lowMTNonIsoTauAnalyzerOutputFiles[${i}]}%" -e "s%LOWMTALLTAUANALYZEROUTFILE%${lowMTAllTauAnalyzerOutputFiles[${i}]}%" -e "s%LOWMTISOTAUANALYZEROUTFILE%${lowMTIsoTauAnalyzerOutputFiles[${i}]}%" -e "s%EDMOUTFILE%${EDMOutputFiles[${i}]}%" -e "s%HIGHMTSEQUENCE%process.highMTNonIsoTauAnalysisSequence%" -e "s%LOWMTSEQUENCE%process.lowMTNonIsoTauAnalysisSequence%" -e "s%PUSCENARIO%S10%" -e "s%REWEIGHT%True%" -e "s%BTAGSCALESHIFT%mean%" -e "s%SAMPLE%%" ../${templateCfg} > tauanalyzer_${samples[${i}]}_nonIso_cfg.py
...
exit 0" >> submitWWTauAnalyzerJobs.sh
...
exit 0" >> submitWWAllTauAnalyzerJobs.sh
  ...
  for cut in Iso NonIso
    ...
    for MTBin in high low
      ...
      if [ "\$cut" != "NonIso" ] || [ $reweightOnly -eq 0 ]
          then
          cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHad\${cut}Analysis_\${MTBin}MT_WW_${version}.root /mnt/hadoop/store/user/kmtos/data1/`whoami`/WW/analysis/
          rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHad\${cut}Analysis_\${MTBin}MT_WW_${version}.root
      fi
    done
  done
done
...
for sample in \`seq `expr $iBeg + 1` `expr $iEnd + 1`\`
  ...
  for MTBin in high low
    do
    cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHadAnalysis_\${MTBin}MT_WW_${version}.root /mnt/hadoop/store/user/kmtos/data1/`whoami`/WW/analysis/
    rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHadAnalysis_\${MTBin}MT_WW_${version}.root
