...
if [ "$sample" = "WJets" ]
    ...
    first_file=$5
    last_file=$6
    prefix="file:/mnt/hadoop/store/user/kmtos/"
    EOSDir=$7
    filePrefix=$8
    file_list=`cmsLs $EOSDir | grep root | awk '{ print $5 }'`
    ...
    for iFile in `seq ${first_file} ${last_file}`
	  ...
	  if [ `expr $iFile % $nFilesPerJob` != "0" ] && [ "$iFile" != "$last_file" ]
          ...
          #create cfg and sh files if you're at the max number of files per job
              ...
              #Create the condor files
              cat>Csubmit_analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}<<EOF
Universe   = vanilla
Executable = analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}.sh
Output     = analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}
Log        = Csubmit_analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}.log
Output     = Csubmit_analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}.out
Queue
EOF
	      input_file_list="${input_file_list}${file_name}"
	      ...
	      chmod a+x analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}.sh
              echo "generateAnalyzeSelectionCfgs.sh"
              condor_submit Csubmit_analyzeSelectionTemplate_${discriminator}_${selection}_${nJobs}
	      input_file_list=""
	      nJobs=`expr $nJobs + 1`
	  fi
      fi
