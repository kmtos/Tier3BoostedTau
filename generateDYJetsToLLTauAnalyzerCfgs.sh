...
if [ $# -gt 3 ]
    then
    echo "Usage: ./generateDYJetsToLLTauAnalyzerCfgs.sh <version> <template cfg> [reweightOnly]"
    exit 0
fi
...
#input file prefix
begInputFilePrefix="file:/mnt/hadoop/store/user/kmtos/NMSSM_Files/DYJetsToLL_M-"
...
EDMOutputFilePrefix="file:/mnt/hadoop/store/user/kmtos/data1/kmtos/DYJetsToLL/EDM_files/"
...
nonIsoReweightTauAnalyzerOutputFiles=( "${tauAnalyzerOutputFilePrefix}muHadNonIsoReweightAnalysis_DYJetsToLL_M-10To50_${version}.root" "${tauAnalyzerOutputFilePrefix}muHadNonIsoReweightAnalysis_DYJetsToLL_M-50_${version}.root" )
...
#generate script that submits all iso+nonIso+reweight jobs to LSF (Now Condor Instead)
...
exit 0" >> submitDYJetsToLLAllTauAnalyzerJobs.sh
chmod a+x submitDYJetsToLLAllTauAnalyzerJobs.sh

#generata script that copies all iso+nonIso+reweight files locally from EOS
  ...
  for cut in Iso NonIso
    ...
    for MTBin in high low
      ...
      if [ "\$cut" != "NonIso" ] || [ $reweightOnly -eq 0 ]
          then
          cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHad\${cut}Analysis_\${MTBin}MT_DYJetsToLL_M-\${sample}_${version}.root /mnt/hadoop/store/user/kmtos/data1/kmtos/DYJetsToLL/analysis/
          rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHad\${cut}Analysis_\${MTBin}MT_DYJetsToLL_M-\${sample}_${version}.root
      fi
    done
  done
done
...
cat <<EOF > copyAllDYJetsToLLFromEOS.sh
#!/bin/bash
...
for sample in "10To50" "50"
  ...
  for MTBin in high low
    do
    cp -f /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHadAnalysis_\${MTBin}MT_DYJetsToLL_M-\${sample}_${version}.root /mnt/hadoop/store/user/kmtos/data1/kmtos/DYJetsToLL/analysis/${MTBin}MT_DYJetsToLL_M-\${sample}_${version}.root /data1/`whoami`/DYJetsToLL/analysis/
    rm /home/kmtos/NMSSM_Analysis/CMSSW_5_3_11/src/BoostedTauAnalysis/TauAnalyzer/test/${version}/muHadAnalysis_\${MTBin}MT_DYJetsToLL_M-\${sample}_${version}.root
done
done

exit 0
EOF
chmod a+x copyAllDYJetsToLLFromEOS.sh
